...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, to this crew only on branch plus code-identity evidence, then keeps that run-step authoritative even if the pane has closed.
On one branch a live rerun outranks the terminal run it superseded, an active run is not disqualified by a pipeline-side head this worktree never fetched, and a live run that cannot be bound to the worktree withholds attribution entirely rather than reviving the superseded terminal run.
The script header owns the exact run-head verdict and preference rules.
...
For ship and scout work, `fm-spawn.sh` leases that worktree with `treehouse get --lease` in its own shell rather than reading the path back off the terminal, returns the lease on every failure path until the published task record owns it, and refuses to launch unless the leased path is a real git worktree root that is distinct from the project primary checkout.
