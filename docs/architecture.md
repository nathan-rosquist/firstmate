...
For ship and scout work, `fm-spawn.sh` leases that worktree with `treehouse get --lease` in its own shell rather than reading the path back off the terminal, and refuses to launch unless the leased path is a real git worktree root that is distinct from the project primary checkout.
It returns the lease on every failure path before the launch command is sent, including a task-record rollback that un-publishes the record; once the launch command has gone out an agent may already be running inside the worktree, so a rollback keeps the lease and names it, and only `bin/fm-teardown.sh`'s guarded return hands it back.
