...
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, and the per-startup-phase confirmation bound, which confirms a child whose phases each fit one window while their sum does not and still tears down a child that stalls inside one phase, naming that phase in the ledger.
