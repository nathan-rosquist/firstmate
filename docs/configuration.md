...
FM_INACTIVE_RECONCILE_BUDGET_SECS=10  # 1..30-second deadline per scan invocation, owned outright by whichever of its two passes is due rather than shared between them; a session-start scan in a secondmate home runs both passes and spends two such deadlines; the wedged-scan kill backstop follows one second after the last deadline that invocation can hold
...
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm allows a fresh watcher in each startup phase (fork, lock, identity, beacon) before reporting FAILED, where fork means the child is running but has not claimed the lock yet and is the phase named when no lock ever appears; the same value also bounds the wait for an already published healthy successor, spent there as one whole window rather than per phase; default 30 on Git Bash/MSYS
FM_ARM_ATTACH_POLL=0.5  # seconds between checks while fm-watch-arm is attached to an existing healthy watcher cycle
FM_OPENCODE_ARM_READY_TIMEOUT_MS=12000   # milliseconds the OpenCode primary watcher plugin waits for an arm attempt to report started, healthy, wake, or failure; the 35000 Windows default covers one MSYS confirmation window and not the arm's per-phase budget, so this adapter can still retire an arm whose child is progressing through its startup phases
FM_PI_ARM_READY_TIMEOUT_MS=12000   # milliseconds the Pi watcher extension waits for a successor arm to report started or attached; the 35000 Windows default covers one MSYS confirmation window and not the arm's per-phase budget, so this adapter can still retire an arm whose child is progressing through its startup phases
