...
FM_ARM_CONFIRM_TIMEOUT=10   # seconds fm-watch-arm allows a fresh watcher in each startup phase (fork, lock, identity, beacon) before reporting FAILED, where fork means the child is running but has not claimed the lock yet and is the phase named when no lock ever appears; default 30 on Git Bash/MSYS
