...
A pane can still disappear between verification and the operation; downstream submit, worktree-arrival confirmation, and stale detection report that narrow race rather than treating exit 0 as success.
...
`pane_cwd` follows a top-level shell `cd` but not a foreground subshell such as the one an interactive `treehouse get` opens.
The current-path read therefore sends begin and end markers around `pwd`, captures the marked block, and joins wrapped path lines, which stays correct for a pane sitting inside such a subshell.
This active probe is scoped to spawn-time worktree-arrival confirmation and is not advertised as a general live-cwd API.
...
- Reading a pane's live cwd requires the spawn-time marker probe.
