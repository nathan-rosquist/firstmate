...
kill "$holder_guard" 2>/dev/null || true
# The guard is killed on purpose, so its 143 is the expected answer here, not a
# failure: `wait` must not be allowed to end the run under an errexit shell.
wait "$holder_guard" 2>/dev/null || true
