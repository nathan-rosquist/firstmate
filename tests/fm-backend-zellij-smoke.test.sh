...
# `treehouse get` does). Verified real bug: zellij's `pane_cwd` JSON field stays
# frozen at wherever the pane's shell was when it launched the subshell as a
...
# pwd-probe (docs/zellij-backend.md) is what keeps this op honest for a pane
# parked inside such a subshell, so it must be proven against a real subshell,
